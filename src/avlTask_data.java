...
public class avlTask_data<E extends Comparable<E>> {
    ...
    public static void main(String[] args) throws IOException {
        ...
        avlTask_data<Integer> distTree = new avlTask_data<>();

        int lastX = 0, lastY = 0, lastDist;
        int closest = 0;
        int min = 101, max = -101;
        ...
        while (scanner.hasNextInt()) {
            ...
            for (int i = 0; i < 60000 && scanner.hasNextInt(); i++) {
                x = scanner.nextInt();
                y = scanner.nextInt();
        ...
        lastDist = (int) Math.round(Math.sqrt(Math.pow(lastX, 2) + Math.pow(lastY, 2)));

        distTree.add(lastDist);

        Node<Integer> node = distTree.find(lastDist);
        Node<Integer> leftN = distTree.findMax(node.left);
        Node<Integer> rightN = distTree.findMin(node.right);

        if (leftN == null && rightN == null) {
            closest = node.parent.value;
        } else if (leftN != null && rightN == null) {
            closest = leftN.value;
        } else if (rightN != null && leftN == null) {
            closest = rightN.value;
        } else {
            Integer leftBig = leftN.value, rightSmall = rightN.value;
            if (lastDist - leftBig < rightSmall - lastDist) {
                closest = leftBig;
            } else {
                closest = rightSmall;
            }
